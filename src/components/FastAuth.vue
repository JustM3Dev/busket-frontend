    ...
    >
      <v-card color="white" :loading="loading">
        <v-card-title>
          ...
          FastAuth | {{ currentScreen !== 'pick' ? currentScreen[0].toUpperCase() + currentScreen.slice(1) : 'Home' }}
        </v-card-title>
        <v-card-subtitle>The fastest way to auth on Busket.</v-card-subtitle>
        <v-card-text>
          <div v-if="currentScreen === 'pick'">
            <div class="mb-2" v-if="auth !== null">Currently signed in as {{ auth.user.username }}.
            </div>
            <v-btn outlined block color="primary" class="mb-2" @click="logOut" v-if="loggedIn">Logout</v-btn>
            <v-btn outlined block color="primary" class="mb-2" @click="currentScreen = 'login'" v-else>Login</v-btn>
            <v-btn outlined block color="blue" class="mb-2" @click="currentScreen = 'signup'">Signup</v-btn>
            <v-btn outlined block color="red" class="mb-4" v-show="loggedIn">Delete</v-btn>
            <v-spacer></v-spacer>
            <v-btn text block color="primary" @click="$emit('close')">Close</v-btn>
          </div>
